...
- [Selenium Boot](https://seleniumboot.com) - Opinionated Java framework built on Selenium WebDriver and TestNG, published on Maven Central under Apache 2.0. A single dependency plus a `selenium-boot.yml` file supplies the parts most teams hand-build: per-thread driver lifecycle, centralised explicit waits, test retry, screenshots, an HTML report, JUnit XML output and CI pass-rate gates. It does not wrap or hide Selenium — the WebDriver instance stays directly accessible, so existing Selenium code keeps working.
